#include "../engine.hpp"
#include "../../tests/ut.hpp"
#include <random>
#include <vector>

namespace ut = boost::ut;

ut::suite mateTest = [] {
  using namespace ut;

  "1. mate in 1"_test = []{
    const std::string FEN = "rnbqkb1r/ppp1pp1p/6p1/4N3/2B5/2n5/PPPP1PPP/R1BQK2R w KQkq - 0 7";
    const chess::Move EXPECTED_MOVE{chess::parseSquare("c4"), chess::parseSquare("f7")};
    bool whiteToMove = true;

    engine::Engine e(FEN);
    e.depth = 4;

    auto moves = e.generateLegalMoves(e.board);
    chess::Move bestMove = e.getBestMove(moves, whiteToMove);

    expect(bestMove.from == EXPECTED_MOVE.from && bestMove.to == EXPECTED_MOVE.to)
      << "Expected mate move " << chess::squareToString(EXPECTED_MOVE.from) << chess::squareToString(EXPECTED_MOVE.to)
      << ", but got " << chess::squareToString(bestMove.from) << chess::squareToString(bestMove.to) << '\n';
  };
  
  "2. mate in 1"_test = []{
    const std::string FEN = "8/pp1Rrkp1/2p4p/5K2/2P1r3/1P2N1P1/P6P/3R4 b - - 0 1";
    const chess::Move EXPECTED_MOVE{chess::parseSquare("g7"), chess::parseSquare("g6")};
    bool whiteToMove = false;

    engine::Engine e(FEN);
    e.depth = 4;

    auto moves = e.generateLegalMoves(e.board);
    chess::Move bestMove = e.getBestMove(moves, whiteToMove);

    expect(bestMove.from == EXPECTED_MOVE.from && bestMove.to == EXPECTED_MOVE.to)
      << "Expected mate move " << chess::squareToString(EXPECTED_MOVE.from) << chess::squareToString(EXPECTED_MOVE.to)
      << ", but got " << chess::squareToString(bestMove.from) << chess::squareToString(bestMove.to) << '\n';
  };

  "3. mate in 1"_test = []{
    const std::string FEN = "2kr1bnr/1p1nq1p1/B1p3b1/3p2p1/Q2PpBP1/2P3N1/PP3P1P/R3K2R w - - 0 1";
    const chess::Move EXPECTED_MOVE{chess::parseSquare("a4"), chess::parseSquare("c6")};
    bool whiteToMove = true;

    engine::Engine e(FEN);
    e.depth = 4;

    auto moves = e.generateLegalMoves(e.board);
    chess::Move bestMove = e.getBestMove(moves, whiteToMove);

    expect(bestMove.from == EXPECTED_MOVE.from && bestMove.to == EXPECTED_MOVE.to)
      << "Expected mate move " << chess::squareToString(EXPECTED_MOVE.from) << chess::squareToString(EXPECTED_MOVE.to)
      << ", but got " << chess::squareToString(bestMove.from) << chess::squareToString(bestMove.to) << '\n';
  };

  "4. mate in 1 with promotion for white"_test = []{
    const std::string FEN = "6k1/P4ppp/8/8/8/8/8/4K3 w - - 0 1";
    const chess::Move EXPECTED_MOVE{chess::parseSquare("a7"), chess::parseSquare("a8")};
    bool whiteToMove = true;

    engine::Engine e(FEN);
    e.depth = 4;

    auto moves = e.generateLegalMoves(e.board);
    chess::Move bestMove = e.getBestMove(moves, whiteToMove);

    expect(bestMove.from == EXPECTED_MOVE.from && bestMove.to == EXPECTED_MOVE.to)
      << "Expected mate move " << chess::squareToString(EXPECTED_MOVE.from) << chess::squareToString(EXPECTED_MOVE.to)
      << ", but got " << chess::squareToString(bestMove.from) << chess::squareToString(bestMove.to) << '\n';
  };

  "5. mate in 1 with promotion for black"_test = []{
    const std::string FEN = "6k1/8/8/8/8/8/p4PPP/6K1 b - - 0 1";
    const chess::Move EXPECTED_MOVE{chess::parseSquare("a2"), chess::parseSquare("a1")};
    bool whiteToMove = false;

    engine::Engine e(FEN);
    e.depth = 4;

    auto moves = e.generateLegalMoves(e.board);
    chess::Move bestMove = e.getBestMove(moves, whiteToMove);

    expect(bestMove.from == EXPECTED_MOVE.from && bestMove.to == EXPECTED_MOVE.to)
      << "Expected mate move " << chess::squareToString(EXPECTED_MOVE.from) << chess::squareToString(EXPECTED_MOVE.to)
      << ", but got " << chess::squareToString(bestMove.from) << chess::squareToString(bestMove.to) << '\n';
  };

  // ========== MATE IN 2 TESTS ==========

  "1. mate in 2"_test = []{
    const std::string FEN = "3r2k1/1qp3p1/pp6/2p1P1N1/P2n2P1/1P1Qpr2/2P3K1/7R w - - 0 1";

    const chess::Move EXPECTED_FIRST_MOVE{chess::parseSquare("h1"), chess::parseSquare("h8")};

    std::vector<chess::Move> possibleResponses = {
      {chess::parseSquare("g8"), chess::parseSquare("h8")}
    };

    bool whiteToMove = true;

    engine::Engine e(FEN);
    e.depth = 6;

    auto whiteMoves = e.generateLegalMoves(e.board);
    chess::Move firstMove = e.getBestMove(whiteMoves, whiteToMove);

    expect(firstMove.from == EXPECTED_FIRST_MOVE.from && firstMove.to == EXPECTED_FIRST_MOVE.to)
      << "Expected first move " << chess::squareToString(EXPECTED_FIRST_MOVE.from) << chess::squareToString(EXPECTED_FIRST_MOVE.to)
      << ", but got " << chess::squareToString(firstMove.from) << chess::squareToString(firstMove.to) << '\n';

    chess::Board::MoveState state1;
    e.board.doMove(firstMove, state1);

    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, possibleResponses.size() - 1);
    chess::Move opponentResponse = possibleResponses[dis(gen)];

    chess::Board::MoveState state2;
    e.board.doMove(opponentResponse, state2);

    auto finalMoves = e.generateLegalMoves(e.board);
    chess::Move matingMove = e.getBestMove(finalMoves, whiteToMove);

    chess::Board::MoveState state3;
    e.board.doMove(matingMove, state3);

    e.updateGameResult();
    bool isMate = e.isMate();

    expect(isMate) << "Expected checkmate but position is not mate\n";
  };

  "2. mate in 2"_test = []{
    const std::string FEN = "2kr3r/p4p2/4b2p/3pq3/Q4p2/2P5/P3B1PP/1R4K1 w - - 0 1";

    const chess::Move EXPECTED_FIRST_MOVE{chess::parseSquare("a4"), chess::parseSquare("c6")};

    std::vector<chess::Move> possibleResponses = {
      {chess::parseSquare("e5"), chess::parseSquare("c7")}
    };

    bool whiteToMove = true;

    engine::Engine e(FEN);
    e.depth = 6;

    auto whiteMoves = e.generateLegalMoves(e.board);
    chess::Move firstMove = e.getBestMove(whiteMoves, whiteToMove);

    expect(firstMove.from == EXPECTED_FIRST_MOVE.from && firstMove.to == EXPECTED_FIRST_MOVE.to)
      << "Expected first move " << chess::squareToString(EXPECTED_FIRST_MOVE.from) << chess::squareToString(EXPECTED_FIRST_MOVE.to)
      << ", but got " << chess::squareToString(firstMove.from) << chess::squareToString(firstMove.to) << '\n';

    chess::Board::MoveState state1;
    e.board.doMove(firstMove, state1);

    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, possibleResponses.size() - 1);
    chess::Move opponentResponse = possibleResponses[dis(gen)];

    chess::Board::MoveState state2;
    e.board.doMove(opponentResponse, state2);

    auto finalMoves = e.generateLegalMoves(e.board);
    chess::Move matingMove = e.getBestMove(finalMoves, whiteToMove);

    chess::Board::MoveState state3;
    e.board.doMove(matingMove, state3);

    e.updateGameResult();
    bool isMate = e.isMate();

    expect(isMate) << "Expected checkmate but position is not mate\n";
  };

};
