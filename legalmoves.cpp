...
namespace chess {
    ...
    bool promotePawn(board cb[DIM][DIM], const unsigned char &player, const int &rank1, const int &file1) noexcept {
        char promotedPawn;
        ...
        switch (std::toupper(promotedPawn)) {
            case 'N':
                return (cb[rank1 - 1][file1 - 1].piece = player | KNIGHT);
            [[unlikely]] case 'B':
                return (cb[rank1 - 1][file1 - 1].piece = player | BISHOP);
            case 'R':
                return (cb[rank1 - 1][file1 - 1].piece = player | ROOK);
            [[likely]] case 'Q':
                return (cb[rank1 - 1][file1 - 1].piece = player | QUEEN);
            default:
    ...
    bool rookMove(gameStatus &gs, const int &rank1, const int &file1, const int &rank2, const int &file2, bool isQueen) noexcept {
        ...
        if (rank1 < rank2)
            for (int trank = rank1 + 1; trank < rank2; ++trank)
                if (gs.chessboard[trank - 1][file2 - 1].piece)
                    return false; // the last if condition is an abbrevation for: "cb[trank - 1][file2 - 1].piece" = "cb[trank - 1][file2 - 1].piece != EMPTY"
        if (rank1 > rank2)
            for (int trank = rank1 - 1; trank > rank2; --trank)
                if (gs.chessboard[trank - 1][file2 - 1].piece)
                    return false;
        if (file1 < file2)
            for (int tfile = file1 + 1; tfile < file2; ++tfile)
                if (gs.chessboard[rank2 - 1][tfile - 1].piece)
                    return false;
        if (file1 > file2)
            for (int tfile = file1 - 1; tfile > file2; --tfile)
                if (gs.chessboard[rank2 - 1][tfile - 1].piece)
...
*/
        ...
        switch (gs.chessboard[rank1 - 1][file1 - 1].piece & PIECEMASK) { //! check which piece i'm using and get the legal moves
            ...
            case (PAWN):
                if (file1 != file2) { // check if file2 is different to file1 (pawns can't move diagonally but they can take pieces on the square next to the diagonal)
                    ...
                    if (!((file2 == file1 + 1) || (file2 == file1 - 1)) || gs.chessboard[rank2 - 1][file2 - 1].piece == EMPTY)
                        return false;
                } else if (gs.chessboard[rank2 - 1][file2 - 1].piece != EMPTY)
                ...
                switch (gs.player) {
                    case (WHITE): 
                        ...
                        if (rank1 == 2)
                            return (rank2 == 3 || ((rank2 == 4) && (!gs.chessboard[2][file2 - 1].piece))); // if pawn is on starting position then it can move only 1 or 2 squares forward
                        if (rank2 != rank1 + 1)
                    ...
                    case (BLACK):
                        if (rank1 == 7)
                            return (rank2 == 6 || ((rank2 == 5) && (!gs.chessboard[5][file2 - 1].piece)));
                        if (rank2 != rank1 - 1)
                            return false;
                        if (rank2 != 1)
                            return true;
            ...
            case (QUEEN):
                return (bishopMove(gs.chessboard, rank1, file1, rank2, file2) || rookMove(gs, rank1, file1, rank2, file2, true)) ? true : false;
    ...
    //     for (long long unsigned int i = 0; i < gs.wherePieceAt.size(); ++i) {

    //         // create 2 variables for file and rank every iteration
    //         int file = fromCharToInt(gs.wherePieceAt[i].file) - 1;
    //         int rank = gs.wherePieceAt[i].rank - 1;
    ...
    //                 }
    //                 for (int it_file = file + 1; it_file < DIM; ++it_file) {
    ...
    //                     return;
    //                 }
    //                 for (int it_rank = rank + 1; it_rank < DIM; ++it_rank) {
    //                     if (gs.chessboard[file][it_rank].piece == EMPTY) {
    //                         updateCoordsV(gs, (char)(file + 'a'), rank, (char)(file + 'a'), it_rank);
    //                         return;
    //                     }
    ...
    //                     return;                            
    //                 }
    //                 for (it_file = file + 1, it_rank = rank - 1; it_file < DIM && it_rank >= 0; ++it_file, --it_rank) {
    //                     if (gs.chessboard[it_file][it_rank].piece == EMPTY) {
    //                         return;
    //                     }
    //                     if ((gs.chessboard[it_file][it_rank].piece & PLAYERMASK) != gs.player) {
    ...
    //                     return;                            
    //                 }
    //                 for (it_file = file - 1, it_rank = rank + 1; it_file >= 0 && it_rank < DIM; --it_file, ++it_rank) {
    //                     if (gs.chessboard[it_file][it_rank].piece == EMPTY) {
    //                         return;
    //                     }
    //                     if ((gs.chessboard[it_file][it_rank].piece & PLAYERMASK) != gs.player) {
    ...
    //                     return;                            
    //                 }
    //                 for (it_file = file + 1, it_rank = rank + 1; it_file < DIM && it_rank < DIM; ++it_file, ++it_rank) {
    //                     if (gs.chessboard[it_file][it_rank].piece == EMPTY) {
    //                         return;
    //                     }
    //                     if ((gs.chessboard[it_file][it_rank].piece & PLAYERMASK) != gs.player) {
