        ...
        switch (cb[rank1 - 1][file1 - 1].piece & PIECEMASK) { //! check which piece i'm using and get the legal moves
            ...
            case (PAWN):
                ...
                if (file1 != file2) { // check if file2 is different to file1 (pawns can't move diagonally but they can take pieces on the square next to the diagonal)
                    if (!((file2 == file1 + 1) || (file2 == file1 - 1)) || cb[rank2 - 1][file2 - 1].piece == EMPTY) return false;
                } else if (cb[rank2 - 1][file2 - 1].piece != EMPTY) return false; // can't move in a square already occupied if the pawn isn't taking any pieces
                ...
                } return promotePawn(cb, player, rank1, file1);                // if we arrive here it means the only option left was the promotion
